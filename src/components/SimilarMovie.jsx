...
import { getSimilarMovie } from "../services/MovieService";

export default function Similar({ id }) {
  const [similars, setSimilars] = useState([]);
  const getSimilar = async () => {
    const data = await getSimilarMovie(id);
    setSimilars(data);
  };
  useEffect(() => {
    getSimilar();
    ...
    <>
      <div className="container mx-auto mt-10">
        ...
        <div className="mt-4 flex items-center gap-4">
          ...
          >
            {similars.length <= 0 ? (
              <h2>Unfortunately there are no similar movies</h2>
            ) : (
              similars?.map((trending) => {
                return (
                  <SwiperSlide
